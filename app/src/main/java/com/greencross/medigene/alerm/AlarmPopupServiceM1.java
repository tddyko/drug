...
/**
 * 매직데이 가임기 알람 호출시 실행되는 Service<br/>
 * 메시지를 세팅하고 AlarmPopAcitivity를 호출한다.
 */
public class AlarmPopupServiceM1 extends Service {
	...
	public void onStart(Intent intent, int startId) {
		...
		if(Utils.TEST){
			Log.i(Utils.TAG, "AlarmPopupServiceM1");
		}
		Utils.pref = getSharedPreferences("pref", Activity.MODE_PRIVATE);
		if(Utils.pref.getBoolean("magicalram" , true)){
			notifiContent = getString(R.string.str_0006);
			popupContent = getString(R.string.str_0006);
